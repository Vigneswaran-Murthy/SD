#!/bin/bash
###############################################################################
# NetBackup Client Auto-Install Script 
# Purpose : Automate NBU Client Installation using local installer package
# Author  : Vigneswaran Murthy
# Version : 3.0 (Variable-driven Dynamic Version)
###############################################################################
...
cert_info=$2     # Certificate passed as argument
...
###############################################################################
function pre_chk() {
  echo -e "NetBackup Client Installation (Variable-driven)\nDate: $(date)\n" > "${out_log}"
  do_ln
  ...
  # Step 6: Token and certificate
  echo "${tkn_info}" > "${tkn_fil}"
  echo "${cert_info}" > "${cert_fil}"

  tkn=$(head -1 "${tkn_fil}")
  cacert=$(head -1 "${cert_fil}")
  [[ -z "${tkn}" ]] && echo "Invalid token" >> "${out_log}" && ex_it 98
  [[ -z "${cacert}" ]] && echo "Invalid certificate" >> "${out_log}" && ex_it 98
  echo -e "\n6) Token and certificate validated" >> "${out_log}"
...
SERVER = ${mst_FQDN}
MACHINE_ROLE = CLIENT
CA_CERTIFICATE_FINGERPRINT = ${cacert}
AUTHORIZATION_TOKEN = ${tkn}
...
# Purpose : Generate bp.conf dynamically from variable file
###############################################################################
function create_bp_conf() {
