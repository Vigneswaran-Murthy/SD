...
if [[ "$INSTALL_PACKAGES" =~ ^[Yy]$ ]]; then
    ...
    if [ $? -eq 0 ]; then
        TASK_STATUS["Domain Join Package Installation"]="✅ Completed"
    else
        TASK_STATUS["Domain Join Package Installation"]="❌ Failed"
    fi
fi
...
if [[ "$JOIN_DOMAIN" =~ ^[Yy]$ ]]; then
    echo "Attempting to join AD domain..."
    ...
    if [ $? -eq 0 ]; then
        TASK_STATUS["AD Domain Join"]="✅ Completed"
    else
        TASK_STATUS["AD Domain Join"]="❌ Failed"
    fi
fi
...
if [[ "$CREATE_MADHU_USER" =~ ^[Yy]$ ]]; then
    ...
    while true; do
        ...
        if [[ "$XSERVICE_PASS" != "$XSERVICE_CONFIRM" ]]; then
            ...
            if [ $? -eq 0 ]; then
                TASK_STATUS["xservice User Creation"]="✅ Completed"
            else
                TASK_STATUS["xservice User Creation"]="❌ Failed"
            fi
            break
        fi
    done
